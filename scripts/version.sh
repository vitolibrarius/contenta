...
if [ ! -d .git ]; then
	echo "Could not find .git directory, are you in the repository root?"
	exit 1
fi

BRANCH=`git status -bs | tr '.' ' ' | awk '{print $2}'`
if [ ! "$BRANCH" = "master" ]; then
	echo "You seem to be on a branch [$BRANCH], switch to master to change versions"
	exit 1
fi
